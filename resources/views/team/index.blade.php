                ...
                <table class="w-full text-left border-collapse">
                    <thead>
                        <tr class="bg-slate-900 text-white text-[11px] font-extrabold uppercase tracking-wider">
                            <th class="py-3 px-4 min-w-[220px] sticky left-0 z-20 bg-slate-900 border-r border-slate-800 shadow-[4px_0_8px_-2px_rgba(0,0,0,0.3)]">System Role</th>
                            @foreach($features as $fKey => $fLabel)
                                <th class="py-3 px-2.5 text-center whitespace-nowrap min-w-[120px]" title="{{ $fLabel }}">
                                    {{ $fLabel }}
                                </th>
                            @endforeach
                            <th class="py-3 px-3 text-center whitespace-nowrap">Quick Actions</th>
                        </tr>
                    </thead>
                    <tbody class="divide-y divide-slate-100 text-xs font-medium">
                        ...
                        <tr class="hover:bg-slate-50/80 transition-all {{ $isOwner ? 'bg-amber-50/30' : '' }}">
                            <td class="py-2.5 px-4 sticky left-0 z-10 border-r border-slate-200/80 shadow-[4px_0_8px_-2px_rgba(0,0,0,0.05)] {{ $isOwner ? 'bg-amber-50' : 'bg-white' }}">
                                <div class="font-bold text-slate-900 flex items-center gap-2">
                                    <span>{{ $rLabel }}</span>
                                    @if($isOwner)
                                        <span class="px-1.5 py-0.5 rounded text-[9px] font-extrabold bg-amber-500 text-white">Full Access</span>
                                    @endif
                                </div>
                            ...
                            <td class="py-2.5 px-2.5 text-center">
                                @if($isOwner)
                                    <i class="fa fa-check-circle text-emerald-500 text-sm" title="Super Admin always has access"></i>
                                @else
                            ...
                            <td class="py-2.5 px-3 text-center">
                                @if(!$isOwner)
                                    <div class="inline-flex items-center gap-1">
                                        <button type="button" onclick="toggleRoleCheckboxes('{{ $rKey }}', true)" class="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-700 font-bold rounded text-[10px]">Select All</button>
                                        <button type="button" onclick="toggleRoleCheckboxes('{{ $rKey }}', false)" class="px-2 py-0.5 bg-slate-100 hover:bg-slate-200 text-slate-500 font-bold rounded text-[10px]">Clear</button>
                                    </div>
                                @else
